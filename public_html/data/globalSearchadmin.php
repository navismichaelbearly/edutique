...
if($_POST['mag'] != '' && $_POST['mag2'] == '' && $_POST['garticle'] == '' && $_POST['gactivity'] == '' && $_POST['gall'] == '' && $_POST['gwordbank'] == '' && $_POST['gusers'] == '')
{
  ...
  if ($stmt = $mysqli->prepare("SELECT DISTINCT(a.article_title), a.article_image,a.article_id,a.mag_id FROM edu_article a where a.article_title like ? or a.article_content like ?")) {
		
	 $stmt->bind_param("ss", $param_article_title, $param_article_content);
		 // Set parameters
	...
	if($total_rows != 0){echo "<p style='font-size:16px'>Articles(".$total_rows.")</p>";}
	 ...
	 while ($stmt->fetch()) {
		// if($mag_issue =='22/23'){$link ='magzine-detail.php';} else if($mag_issue =='18'){$link ='magzine-details.php';}
	     echo  "<div class='col-md-2-5 normaltext' align='center' style='margin:0 20px 20px 0; padding:10px; background-color:#ffffff'><a href='article-detail-admin.php?artID=".$article_id."&actID=0&magID=".$mag_id."'><img src='".$article_image."' width='200' height='265' style='border:1px solid #CCCCCC; margin-bottom:5px;'></a><br>".$article_title."</div>";
	...
	echo "</div>";
	//------------- for activity--------------------------
	if ($stmt = $mysqli->prepare("SELECT DISTINCT(a.activity_title), a.image_path,a.article_id,a.mag_id,a.activity_id FROM edu_activity a where a.activity_title like ? or a.activity_content like ?")) {
		
	 $stmt->bind_param("ss", $param_activity_title, $param_activity_content);
		 // Set parameters
	...
	if($total_rows != 0){echo "<p style='font-size:16px'>Activities(".$total_rows.")</p>";}
	 ...
	 while ($stmt->fetch()) {
		// if($mag_issue =='22/23'){$link ='magzine-detail.php';} else if($mag_issue =='18'){$link ='magzine-details.php';}
	     echo  "<div class='col-md-2-5 normaltext' align='center' style='margin:0 20px 20px 0; padding:10px; background-color:#ffffff'><a href='activity-detail-admin.php?artID=".$article_id."&actID=".$activity_id."&magID=".$mag_id."'><img src='".$activity_image."' width='200' height='265' style='border:1px solid #CCCCCC; margin-bottom:5px;'></a><br>".$activity_title."</div>";
	...
	//------------- for users--------------------------
	if ($stmt = $mysqli->prepare("SELECT first_name, last_name, school_name, class_name,a.user_id,b.class_id FROM edu_users a inner join  edu_user_school_level_class b on a.user_id = b.user_id inner join edu_school c on b.school_id=c.school_id inner join edu_class d on b.class_id=d.class_id where  a.first_name like ? or a.last_name like ? and user_type_id=? group by first_name, last_name ")) {
		
	  $stmt->bind_param("sss",  $param_first_name, $param_last_name, $param_user_type_id);
		 // Set parameters  
