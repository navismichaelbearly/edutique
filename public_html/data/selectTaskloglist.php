...
if($_POST['asssigned'] != '' || $_POST['selectTaskid'] == 'All' || $_POST['levelclassid'] == 'Allclasses')
{
     ...
     {
	    $mainSql = "SELECT a.task_id,a.due_date,c.article_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id  inner join edu_article c on a.article_id=c.article_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id=? ".$varDuedate." GROUP BY b.task_id";	 
	 }else if($_POST['selectTaskid'] == 2){
	    $mainSql = "SELECT a.task_id,a.due_date,c.activity_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id inner join edu_activity c on a.activity_id=c.activity_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id!=? ".$varDuedate." GROUP BY b.task_id";
	 }else if($_POST['selectTaskid'] == 3){
	    $varDuedate ="and a.lockitem=1";
	    $mainSql = "SELECT a.task_id,a.due_date,c.article_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id  inner join edu_article c on a.article_id=c.article_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id=? ".$varDuedate." GROUP BY b.task_id UNION SELECT a.task_id,a.due_date,c.activity_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id inner join edu_activity c on a.activity_id=c.activity_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id !=? ".$varDuedate." GROUP BY b.task_id";
	 ...
	 else if($_POST['levelclassid'] !='Allclasses' )
     {
	    ...
	    $varLevelclass =' and e.level=? and d.class_name=?';
	   $mainSql = "SELECT a.task_id,a.due_date,c.article_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id  inner join edu_article c on a.article_id=c.article_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id=? ".$varLevelclass." GROUP BY b.task_id UNION SELECT a.task_id,a.due_date,c.activity_title, d.class_name, e.level,count(if(b.task_stages='Completed',1,null)) as comp_count,count(if(b.task_stages='Overdue',1,null)) as overd_count, count(b.task_id) as tottid, a.article_id,a.activity_id,a.mag_id FROM edu_task a inner join edu_user_task b on a.task_id=b.task_id inner join edu_activity c on a.activity_id=c.activity_id inner join edu_class d on a.class_id = d.class_id inner join edu_levels e on a.level_id=e.level_id WHERE assigned_by=? and a.activity_id !=? ".$varLevelclass." GROUP BY b.task_id";
	   }
      ...
      if ($stmt = $mysqli->prepare($mainSql)) {    
		  ...
		  if($_POST['selectTaskid'] == 1)
         {
		   $stmt->bind_param("ss", $param_assigned_by,$param_actid);
		 }else if($_POST['selectTaskid'] == 2){
		   $stmt->bind_param("ss", $param_assigned_by2,$param_actid2);
		  }else if($_POST['selectTaskid'] == 3){
		   $stmt->bind_param("ssss", $param_assigned_by,$param_actid,$param_assigned_by2,$param_actid2); 
		 ...
		 {
		   $stmt->bind_param("ssssssss", $param_assigned_by,$param_actid,$param_level, $param_class,$param_assigned_by2,$param_actid2,$param_level2, $param_class2);
		 }
		 $param_assigned_by = $_SESSION['id'];	
		 $param_assigned_by2 = $_SESSION['id'];
		 $param_actid =0;
		 ...
		 $param_level=$levelclassidexp[0];
		 $param_class=$levelclassidexp[1];
		 $param_level2=$levelclassidexp[0];
		 $param_class2=$levelclassidexp[1];	
		 }
		 $stmt->execute();
		 /* bind variables to prepared statement */
		 $stmt->bind_result($task_id,$due_date, $article_title,$class_name,$level,$comp_count,$overd_count,$tottid, $article_id,$activity_id,$mag_id);
