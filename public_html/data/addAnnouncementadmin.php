...
if ($_POST['asssignInfo'] != '') {
	...
	left join edu_class ec on euslc.class_id = ec.class_id
	 where ec.school_id= ? and ec.class_status = ? and el.level_status = ? ";
	if ($stmt = $mysqli->prepare($sql)) {
		...
		$stmt->bind_param("sss", $param_school_id, $param_class_status, $param_level_status);
		$stmt->execute();
		/* bind variables to prepared statement */
		$stmt->bind_result($level_name, $class_name, $level_id, $class_id);
		$sr = 1;
		$html .=  "<label for='Send To'>Send To:</label><br><table class='table' style='width:100%; background-color:transparent'>";
		while ($stmt->fetch()) {
